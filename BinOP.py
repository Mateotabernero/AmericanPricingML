...
def binOP (r, sigma, S_0, T, K, put_or_call, optionType, M, gamma_par = 1): 
    ...
    if optionType == 'E':
        ...
        for i in reversed(range(M)):
            for j in range(i+1):
                ...
                V[i][j]      = np.maximum(payOff (S[i][j],K,put_or_call), V_cont[i][j])       
...
def generate_tree(r, sigma, S_0, T, K, put_or_call, optionType, M, gamma_par = 1, payOff = payOff): 
    """
